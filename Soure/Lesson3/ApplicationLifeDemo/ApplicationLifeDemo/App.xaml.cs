...
using ApplicationLifeDemo.Resources;
using System.Threading;
using System.IO.IsolatedStorage;
using System.IO;

namespace ApplicationLifeDemo
{
    public partial class App : Application
    {
        ...
        private void Application_Launching(object sender, LaunchingEventArgs e)
        {
            test = new byte[5000];
            //Thread.Sleep(5000);
            Debug.WriteLine("程序正在启动Application_Launching");
        }
        ...
        {
            Debug.WriteLine("程序正在激活Application_Activated");
            if (e.IsApplicationInstancePreserved)
            {
                Debug.WriteLine("休眠状态");
                // Dormant - objects in memory intact
            }
            else
            {
                test = new byte[5000];
                Debug.WriteLine("墓碑状态");
        ...
        {
            Debug.WriteLine("程序正在离开Application_Deactivated");
        }
        ...
        {
            Debug.WriteLine("程序正在关闭Application_Closing");
        }
        ...
        {
            IsolatedStorageFile myStaorage = IsolatedStorageFile.GetUserStoreForApplication();
            IsolatedStorageFileStream isoStream = new IsolatedStorageFileStream("log.txt", System.IO.FileMode.OpenOrCreate, myStaorage);
        ...
        void RootFrame_Navigating(object sender, NavigatingCancelEventArgs e)
        {
            ...
            {
                Debug.WriteLine("第一次导航到程序是休眠的页面");
                reset = true;
                return;
            }
            if (reset && e.IsCancelable && e.NavigationMode == NavigationMode.New)
            {
                Debug.WriteLine("第二次导航到程序的默认首面，但是我们取消掉了");
                e.Cancel = true;
                reset = false;
            }
        }
